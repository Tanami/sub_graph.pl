...
my $options = {
    ...
    'skips_file' => {
        ...
        'default' => 1,
        'order' => 6,
        'verbose' => "Skip files whose first line doesn't m|^\#\!.*/pe
+rl|"
    },
    'graph_name' => {
        'type' => '=s',
    ...
    'special_nodes_fn' => {
        ...
        'order' => 4,
        'verbose' => 'file containing a list of "special" nodes (subro
+utines).'
        },
    'special_node_attr' => {
        'type' => '=s',
    ...
    'perl' => {
        ...
        'order' => 3,
        'verbose' => 'where does perl live?  (used as $PERL -B::Xref <
+fname>)'
        },
    'verbose' => {
        'type' => '+',
    ...
    'special_edges_only' => {
        ...
        'default' => 0,
        'order' => 7,
        'verbose' => "Display only edges to and from nodes in the spec
...
ds_only and to_special_nodes_only."
    ...
    'from_special_nodes_only' => {
        ...
        'default' => 0,
        'order' => 8,
        'verbose' => 'Display only edges from a node in the special no
+de list'
    },
    'to_special_nodes_only' => {
        ...
        'default' => 0,
        'order' => 9,
        'verbose' => 'Display only edges to a node in the special node
+ list'
    },    'no_internal_edges' => {
        'type' => '!',
        'env' => '',
        'default' => 0,
        'order' => 10,
        'verbose' => 'Use this to suppress drawing of edges between tw
...
graph)'
    ...
    'special_subgraph' => {
        ...
        'default' => 0,
        'order' => 7,
        'verbose' => "Build special nodes into a subgraph, with option
+al attributes as specified"
        },
    'subgraph_attributes' => {
        'type' => '=s',
        'env' => '',
...
if (-f $switch->{'skips_file'}) {
    my %skip;
    open F, "$switch->{'skips_file'}" or die "Can't open $switch->{'sk
+ips_file'} for reading";
    while (<F>) {
        chomp;
        foreach (split /\s+/) {
            $skip{$_}++;
...
if (-f $switch->{'special_nodes_fn'}) {
    open F, "$switch->{'special_nodes_fn'}" or die "Can't open $switch
+->{'special_nodes_fn'} for reading";
    while (<F>) {
        chomp;
        foreach (split /\s+/) {
            $want{$_}++;
...
for my $f (@files) {
    ...
    while (<X>) {
        ...
        if (/Subroutine \(?([^()]*)/) {
            $cur_sub = $1;
            $cur_sub = "${f}_MAIN" if $cur_sub eq "main";
            $cur_sub =~ tr/.:/__/; # dot doesn't like punctuation.
        } elsif (/^\s*\&(\w\S+)\s*/) {
            next if $cur_sub eq "definitions"; # seemed like a good id
+ea at the time
            my $s = $1;
            $s =~ s/:/_/g;
            $edges{$cur_sub}->{$s}++;
...
if ($switch->{'verbose'}) {
    ...
    print "I am" . (!$want_to && " not") ." printing edges to special 
+nodes.\n";
    print "I am" . (!$want_from && " not") ." printing edges from spec
+ial nodes.\n";
    print "I am" . (!$want_int && " not") ." printing edges between sp
+ecial nodes.\n";
}
